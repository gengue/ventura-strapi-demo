...
function getResourcePath(resource) {
  switch (resource) {
    ...
    case "roles/count":
      return "users-permissions/roles/count";
    default:
      return resource;
  }
}
...
export default (
   ...
   * @param {String} originalResource Name of the resource to fetch, e.g. 'posts'
   * @param {Object} params The data request params, depending on the type
   * @returns {Object} { url, options } The HTTP request parameters
   */
  const convertDataRequestToHTTP = (type, originalResource, params) => {
    let url = "";
    const options = {};
    const resource = getResourcePath(originalResource);
  ...
  const convertHTTPResponse = (response, type, resource, params) => {
    ...
    switch (type) {
      ...
      case GET_MANY_REFERENCE:
        return {
          data: resource === "roles" ? json.roles : json,
          total,
        };
      case CREATE:
